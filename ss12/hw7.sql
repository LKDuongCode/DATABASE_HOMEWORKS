...
delimiter //
#2
create trigger trg_bf_insert_employees 
before insert on employees for each row
begin
	if new.salary < 500 then
		signal sqlstate '45000' set message_text = 'low salary';
    end if;
    
	if  (select count(dept_id) from departments where dept_id = new.dept_id) = 0 then
        signal sqlstate '45000' set message_text = 'Department not exist.';
    end if;
    if (select count(p.project_id) from projects p 
        join employees e on p.emp_id = e.emp_id
        where e.dept_id = new.dept_id and p.status != 'Completed'
    ) = 0 then
        signal sqlstate '45000'set message_text = 'completed.';
    end if;
end;
// delimiter ;
...
select e.emp_id,e.name as employee_name,d.name as department_name,p.name as project_name,p.status,concat('$', format(e.salary, 2)) as salary,pw.warning_message from employees e
