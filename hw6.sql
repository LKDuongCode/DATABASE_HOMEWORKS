...
#3
select e.full_name, e.department from employees as e 
join (select department 
     from employees 
     group by department 
     having count(employee_id) >= 2) as d on e.department = d.department;
     
#4
select e.full_name, e.department, e.salary from employees as e 
join (select department, max(salary) as max_salary 
     from employees 
     group by department) as d on e.department = d.department and e.salary = d.max_salary;
     
#5
select e.full_name, e.department, e.hire_date from employees as e 
join (select department, min(hire_date) as earliest_hire_date 
     from employees 
     group by department) as d on e.department = d.department and e.hire_date = d.earliest_hire_date;