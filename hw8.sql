...
INSERT INTO products8 (product_name, price, category)
VALUES
...
#2
select p.product_name, w.warehouse_name, i.quantity from inventory as i
 join warehouses as w on w.warehouse_id = i.warehouse_id
 join products8 as p on p.product_id = i.product_id
...
left join products8 as p on p.product_id = i.product_id
where p.category = 'Electronics'
order by i.quantity desc limit 3 offset 1;

#4
select p.product_name, p.price, w.warehouse_name, w.location, i.quantity, s.supplier_name from inventory as i
join products8 as p on i.product_id = p.product_id
join warehouses as w on i.warehouse_id = w.warehouse_id
join suppliers as s on p.product_id = s.product_id
