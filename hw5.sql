...
#2
select u.user_id, u.username, count(p.post_id) as total_post, sum(p.likes_count) as total_likes from users as u
join posts as p on u.user_id = p.user_id
group by u.user_id
having sum(p.likes_count) > 20;

#3
select u.user_id, u.username, p.post_id, p.content, max(p.likes_count) as max_likes from users as u
join posts as p on u.user_id = p.user_id
...
#5
select u.user_id, u.username, p.post_id, p.content, min(likes_count) as min_likes from users as u
join posts as p on u.user_id = p.user_id
group by u.user_id, u.username, p.post_id, p.content
having min(likes_count) < 15;
