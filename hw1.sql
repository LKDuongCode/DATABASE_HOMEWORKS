...
select e.EmployeeID, e.fullname, e.position, e.salary, b.branchname, b.location,  b.PhoneNumber from employees e
join branch b on e.branchid = b.branchid;
...
# ==> nhận xét: việc xóa dữ liệu của bảng từ view là điều có thể nhưng không được khuyến khích sử dụng, thay vào đó có hai hướng 
# 1 xóa trực tiếp từ bảng 
# 2 thay đổi cách mà view hiển thị dữ liệu (ví dụ trong này là thêm điều kiện lọc chi nhánh hà nội ra)
