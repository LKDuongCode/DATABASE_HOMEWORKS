...
#2
select s.student_id, s.name, s.email, sum(c.fee) as total_fee, count(e.course_id) as total_courses from students as s 
...
end as fee_category, e.enrollment_date from students s 
join enrollments as e on s.student_id = e.student_id 
join courses as c on e.course_id = c.course_id 
order by fee_category, s.name;

#4
select c.course_name, c.fee, count(e.student_id) as student_count, sum(c.fee) as total_revenue from courses as c 
