...
case when r.return_date is not null then 'returned' else 'not returned' end as status from customer c
...
select c.customer_id, concat(c.first_name, ' ', c.last_name) as full_name, sum(p.amount) as total_payment from customer c
