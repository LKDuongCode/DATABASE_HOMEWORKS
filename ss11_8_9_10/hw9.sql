...
select c.customerid, concat(c.firstname, c.lastname) as fullname, c.email, sum(i.total) as total_spending from customer c
join invoice i on i.customerid = c.customerid
where year(i.invoicedate) >= '2010' and c.country != 'Brazil'
group by  c.customerid, concat(c.firstname, c.lastname), c.email
having sum(i.total) > 200;

#3
create view view_popular_tracks as
select t.trackid, t.name as track_name, sum(il.quantity) as total_sales from track t
...
#6
select v.customerid, v.fullname, v.email, v.total_spending, c.country from view_high_value_customers v
join customer c on v.customerid = c.customerid
where c.country = 'canada';
explain select v.customerid, v.fullname, v.email, v.total_spending, c.country from view_high_value_customers v
join customer c on v.customerid = c.customerid
where c.country = 'canada';

#7
select v.trackid, v.track_name, v.total_sales, t.unitprice from view_popular_tracks v
join track t on v.trackid = t.trackid
where match (t.name) against ('love');

explain select v.trackid, v.track_name, v.total_sales, t.unitprice from view_popular_tracks v
join track t on v.trackid = t.trackid
where match (t.name) against ('love');
